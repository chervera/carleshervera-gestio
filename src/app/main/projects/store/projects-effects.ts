...
export class ProjectsEffects {
  ...
  loadAll$ = createEffect(() => this.actions$.pipe(
    ...
    switchMap(() => this.projectsApi.getProjects().pipe(
      map(projects => loadAllSuccess({ projects })),
    )),
  ));
  ...
  load$ = createEffect(() => this.actions$.pipe(
    ofType(load),
    pluck('id'),
    switchMap(id => this.projectsApi.getProject(id).pipe(
      map(project => loadSuccess({ project }))
    ))
  ));
